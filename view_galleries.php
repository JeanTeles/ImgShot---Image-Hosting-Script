...
<div id="container">
    ...
    <div id="content">
        <div id="selectable" class="all_images">
            ...
            <?php
            $q = "SELECT name FROM galleries WHERE id = '$gallery_id'";
            $result = mysql_query($q);
            $rowGallery = mysql_fetch_assoc($result);
            echo "<p style='float:right; font-weight:bold; margin-top:-25px;'>" . $rowGallery['name'] . " </p>";



            $qNum = "SELECT id FROM images WHERE id_user = '$_SESSION[user_id]' AND gallery = '$gallery_id'";
...
//PAGINATION SCRIPT
            ...
            $q = "SELECT images.view_id, images.views, images.name, images.date_added, images.ftp, sources.thumb2, sources.img2, ftp_logins.url FROM images
             ...
             WHERE id_user = '$_SESSION[user_id]' AND gallery = '$gallery_id'
        ...
        <div class='moderate_images'>
            <input type="submit" class="moderate_images" value="Moderate Selected Images" /> <br />
            <p><a class='nicelinks' target="_blank" href="slide-<?php echo $gallery_id; ?>.html">Click here to view gallery in slideshow</a></p>
        </div>
    ...
    <div id="pagination">
        ...
        echo "<a href=\"view_galleries.php?id={$gallery_id}&page=$arr[previous]\" class=\"alfabet\">Previous</a> - ";

        foreach($arr['pages'] as $value) {
            echo "<a href=\"view_galleries.php?id={$gallery_id}&page=$value\" class=\"alfabet\">$value</a> ";
        }

        echo " - <a href=\"view_galleries.php?id={$gallery_id}&page=$arr[next]\" class=\"alfabet\"> Next</a> ";
        echo "<a href=\"view_galleries.php?id={$gallery_id}&page=$arr[last]\" class=\"alfabet\">Last</a>";
